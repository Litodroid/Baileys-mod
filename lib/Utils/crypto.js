import { createCipheriv, createDecipheriv, createHash, createHmac, randomBytes, createECDH, createSign, createVerify, constants } from 'crypto';
/* @ts-ignore */
...
export const Curve = {
    ...
    sign: (privateKey, buf) => {
        ...
        // Usar implementación nativa de Node.js como fallback
        try {
            // Asegurarse de que la clave privada está en el formato correcto
            const ecdh = createECDH('prime256v1');
            ecdh.setPrivateKey(privateKey);
            
            // Crear un objeto de firma con el formato de clave correcto
            const sign = createSign('SHA256');
            sign.update(buf);
            
            // Usar la clave privada en el formato correcto
            return sign.sign({
    ...
    verify: (pubKey, message, signature) => {
        ...
        // Usar implementación nativa de Node.js como fallback
        try {
            // Asegurarse de que la clave pública está en el formato correcto
            const ecdh = createECDH('prime256v1');
            ecdh.setPrivateKey(Buffer.alloc(32)); // Clave temporal solo para crear el objeto
            const publicKey = generateSignalPubKey(pubKey);
            
            // Crear un objeto de verificación con el formato de clave correcto
            const verify = createVerify('SHA256');
            verify.update(message);
            
            // Usar la clave pública en el formato correcto
