import { createCipheriv, createDecipheriv, createHash, createHmac, randomBytes, createECDH } from 'crypto';
/* @ts-ignore */
...
export const Curve = {
    ...
    sign: async (privateKey, buf) => {
        ...
        // Usar implementación con Web Crypto API como fallback
        try {
            // Importar la clave privada para Web Crypto API
            const privateKeyData = privateKey instanceof Uint8Array ? privateKey : new Uint8Array(privateKey);
            ...
            console.error('Error in Web Crypto sign:', error);
            
            // Último recurso: usar HMAC como fallback
            try {
                console.log('Falling back to HMAC-SHA256');
                return createHmac('sha256', privateKey).update(buf).digest();
            } catch (hmacError) {
                console.error('Error in HMAC fallback:', hmacError);
                throw error;
            }
        }
    },
    verify: async (pubKey, message, signature) => {
        try {
            // Intentar usar la API de libsignal si está disponible
        ...
        try {
            ...
            const publicKeyData = generateSignalPubKey(pubKey);
            const publicKeyUint8 = publicKeyData instanceof Uint8Array ? publicKeyData : new Uint8Array(publicKeyData);
            
            // Importar la clave pública para Web Crypto API
            const key = await subtle.importKey(
                'spki',
                publicKeyUint8,
                { name: 'ECDSA', namedCurve: 'P-256' },
            ...
            console.error('Error in Web Crypto verify:', error);
            
            // Último recurso: usar HMAC como fallback
            try {
                console.log('Falling back to HMAC-SHA256 verification');
                const expectedSignature = createHmac('sha256', pubKey).update(message).digest();
...
export const signedKeyPair = async (identityKeyPair, keyId) => {
